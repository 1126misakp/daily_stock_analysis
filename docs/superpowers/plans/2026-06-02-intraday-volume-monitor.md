...
## 全局约定（执行前必读，所有 Task 适用）

- **本仓库硬规则（`AGENTS.md` §1）**：
  - commit message **用英文，不添加 `Co-Authored-By`**（本计划各 Task 的 commit 已为英文）。
  - 未经用户明确确认，**不执行 `git push` / `git tag`**；本计划只到本地 `git commit` 为止。
  - 新增配置项**必须同步 `.env.example` 和相关文档**；涉及"通知方式/用户可见能力"变化**必须更新 `docs/CHANGELOG.md`**。本计划用 **Task 6.5** 覆盖。
  - `docs/CHANGELOG.md` 的 `[Unreleased]` 段用**扁平格式** `- [类型] 描述`（类型取 `新功能`/`改进`/`修复`/`文档`/`测试`/`chore`），**禁止新增 `### 子标题**`。
  - commit title **不要**含 `#patch`/`#minor`/`#major`（否则会触发自动 tag）。
- **数据源铁律**：禁止改动 `data_provider/` 的路由/优先级/fetcher 链。取数只经 `manager.get_intraday_kline()`。TickFlow 的限流/重试由数据源层内部 `_call_with_intraday_retry` 完成并在失败后**降级返回 None**；监控器一律按"返回 None = 跳过该股"处理，**不得**在监控器内自行重试 429，**更不得**去 patch `tickflow_fetcher.py`。
- **测试**：unittest 写法、pytest 运行（`python -m pytest tests/xxx.py -v`），与现有 `tests/` 一致。
- 遇到与本计划不符或代码已变化的情况，**停下来核对真实源码**，不要照抄行号。
...
| `.env.example` | 追加 7 个配置项（注释态） | 修改 |
| `docs/CHANGELOG.md` | [Unreleased] 追加一行 | 修改 |
| `docs/intraday-volume-monitor.md` | 功能说明文档 | 创建 |
...
**Files:**
- Modify: `src/config.py`（dataclass 字段区：在 `agent_event_alert_rules_json: str = ""` 行后，约第 728 行；from_env 解析区：在 `agent_event_alert_rules_json=os.getenv('AGENT_EVENT_ALERT_RULES_JSON', ''),` 行后，约第 1535 行。**用 grep 锚点定位，勿照抄行号**）
...
> 注：`_holding_symbols()` 内部已自带 try/except 返回 `[]`；`test_holdings_failure_degrades_to_stock_list` 的 `side_effect` 验证 `resolve_universe` 对异常的兜底。`test_holding_symbols_filters_non_cn` 验证只保留 A 股（cn）持仓。
...
**Files:**
- Modify: `main.py`（`--schedule` 模式的 `background_tasks` 注册块，约 959-976 行，紧邻 `agent_event_monitor` 注册之后；**用 grep 锚点定位**）
...
> `run_immediately=True` 会在容器启动后立即跑一轮，但 `run_once()` 首步做时段 gate，非盘中直接返回零成本、不触发取数（与 agent_event_monitor 行为一致）。
...
## Task 6.5: 同步 .env.example 与文档（AGENTS.md 硬规则）

**Files:**
- Modify: `.env.example`（追加 7 个新配置项，留注释态、带默认值与中文说明）
- Modify: `docs/CHANGELOG.md`（`[Unreleased]` 段扁平追加一行）
- Create: `docs/intraday-volume-monitor.md`（功能说明）

- [ ] **Step 1: `.env.example` 追加配置项**

在 `.env.example` 末尾（或通知/调度相关段之后）追加：

```bash
# ── 盘中分钟级量能监控（独立于告警中心；默认关闭，纯后台 .env 开关）──
# 开启后：交易时段每 N 分钟扫描“自选股∪持仓股(仅A股)”的 5 分钟 K 线成交量，
# 与近 N 交易日同时段均量对比，放量/缩量异动当日同股同类型首次合并推送飞书。
# INTRADAY_VOLUME_MONITOR_ENABLED=false
# INTRADAY_VOLUME_MONITOR_INTERVAL_MINUTES=5
# INTRADAY_VOLUME_SURGE_RATIO=2.0
# INTRADAY_VOLUME_SHRINK_RATIO=0.5
# INTRADAY_VOLUME_BASELINE_DAYS=20
# INTRADAY_VOLUME_BASELINE_MIN_SAMPLES=5
# INTRADAY_VOLUME_INCLUDE_HOLDINGS=true
```

- [ ] **Step 2: `docs/CHANGELOG.md` 的 `[Unreleased]` 段追加一行**

在 `## [Unreleased]` 下方追加（扁平格式，**不要加 `###` 子标题**）：
...
- [ ] **Step 3: 创建 `docs/intraday-volume-monitor.md`**
...
```bash
git add .env.example docs/CHANGELOG.md docs/intraday-volume-monitor.md
git commit -m "docs(intraday-volume): sync .env.example, CHANGELOG and feature doc"
```
